  ...
  <body>
	...
	*{font-family:"微软雅黑";}
	...
	<div class="uc_index_right_div">
		<div id="right_center">
			<a href="<?php echo U('UserCenter/userLeaveTell');?>">我的工单</a>
			<hr>
			<div class="head_r_div">
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				工单编号：<?php echo ($info['id']+1000); ?>&nbsp;&nbsp;&nbsp;
				提交时间：<?php echo ($info["ltime"]); ?>
			<?php if($info["status"] == 'ing'): ?><a onclick="return confirm('确认关闭？')" href="<?php echo U('UserCenter/closeLeaveTell',array('id'=>$info['id']));?>" style="float:right;margin-right:30px;">已完成，关闭</a>
			<?php else: ?>
				<a style="float:right;margin-right:30px;">已完成</a><?php endif; ?>
			</div>
			<div id="tablediv">
				<div id="show_d_content">
					<div id="ne">
						<?php if(is_array($list)): foreach($list as $key=>$li): ?>&nbsp;&nbsp;&nbsp;
								<?php if($li["type"] == 'question'): ?><a style="color:red;"><?php echo ($_SESSION['user']['username']); ?></a>
								<?php else: ?>
								<a style="color:#0099CC;">客服妹妹</a><?php endif; ?>
								<a style="color:black;font-size:10px;">(<?php echo ($li["ltime"]); ?>)</a>
								：<?php echo ($li["question"]); ?>
							<hr id="hr" size="1"><?php endforeach; endif; ?>
					</div>
					<?php if($info["status"] == 'ing'): ?><form action="<?php echo U('UserCenter/showContent');?>" method="post">
							<div id="goone_div">
								继续提交：
								<textarea name="question" placeholder="请填写详细描述"></textarea>
								<input type="hidden" name="ltid" value="<?php echo ($info["id"]); ?>" />
